...
import { DeleteProjectByAdmin, DeleteUserByAdmin } from "../Api/api";
...
const ProjectDeleteAlert = ({
  ...
  const OnClickDeleteUser = async () => {
    const response = await DeleteProjectByAdmin(id);
    if (response.message === "Deleted successfully") {
      Toast.show({
        type: "success", // success, error, info, or any custom type
        position: "top", // top, center, or bottom
        text1: "Delete project",
        text2: "Employee Project successfully",
